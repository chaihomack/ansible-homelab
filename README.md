...
user role:
  currently there is only task for creating user with password and ssh public key in ~/.ssh on remote host by using admin account

apt_proxy:
  adds apt cacher configuration to apt.conf.d/99proxy

zabbix_agent2 role:
  installs and configures zabbix agent

common role:
  installs common packages, updates, upgrades the system and managing reboots when necessary
